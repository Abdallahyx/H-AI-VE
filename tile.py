...
class Tile:
    ...
    def remove_piece(self):
        """
        Remove and return the top piece from the tile

        :return: Removed piece or None
        """
        if self.has_pieces():
            removed_piece = self.pieces.pop(-1)

            # Reset color if no pieces remain
            if not self.has_pieces():
                if type(self) is Inventory_Tile:
                    # Keep original color for inventory tiles
                    pass
                else:
                    self.color = WHITE
            else:
                # Set color to the new top piece
                self.color = self.pieces[-1].color
...
class Inventory_Tile(Tile):
    def __init__(self, coord_pair, axial_coords, radius, color, piece):
        ...
        self.original_color = color  # Store the original color for inventory tiles

    def remove_piece(self):
        """
        Override remove_piece for inventory tiles to maintain color

        :return: Removed piece or None
        """
        if self.has_pieces():
            return self.pieces.pop(-1)
        return None

    def restore_original_color(self):
        """
        Restore the tile to its original color
        """
    ...
    for j in range(0, len(pixel_y)):
        for k in range(0, len(pixel_x)):
            if pixel_y[j] in odd_y:
                tiles.append(
                    Tile(
                        (pixel_x[k] + hex_radius, pixel_y[j]),
